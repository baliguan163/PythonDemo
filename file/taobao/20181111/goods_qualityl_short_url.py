...
if __name__ == "__main__":
    ...
    sql = "SELECT count(*) FROM goods_quality"  # 商品id
    data = ()
    ...
    # for i in range(1, 2):
        # print(i)
        sql = "SELECT * FROM goods_quality WHERE id='%s'"  # 商品id
        data = (i)
        cursor.execute(sql % data)
        results = cursor.fetchall()
        # print(results)
        for data in results:
            ...
            print('总:',sum,'->',i,' 商品推广链接:',longUrl1Len,longUrl1)
            print('总:',sum,'->',i,' 优惠券推广链接',longUrl2Len,longUrl2)
