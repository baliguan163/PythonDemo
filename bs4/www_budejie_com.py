...
def get_pages_url(url,root_dir_2):
    html = get_html(url)
    soup = BeautifulSoup(html, 'lxml')
    yi_list = soup.find_all('div', class_= 'j-r-list')
    # print('yi_list:', yi_list)
    ...
    for i in range(0, len(yi_list)):
        ...
        for j in range(0, len(content)):
            try:
                ...
                file = root_dir_3 + user_name + '.txt'
                isExists = os.path.exists(file)
                if isExists:
                    os.remove(file)

                Out2File(file,user_name)
                Out2File(file, user_href)
                Out2File(file, context_href)
                Out2File(file, context)
                list_img = content[j].find('div', class_='j-r-list-c-img').find('a').find_all('img')
                for m in range(0, len(list_img)):
                    pic_url = list_img[m]['data-original']
                    Out2File(file, pic_url)

                    index = pic_url.rfind('.')
                    pre = pic_url[index:len(pic_url)]
                    pretemp = str(m+1)+ pre
                    print('  图片地址:', pic_url,pretemp)
...
def main():
     ...
     # for j in range(0, 2):
        get_pages_url(pages_url_list[j],root_dir_2)
        ...
        #     root_dir_1 = create_dir(root_dir_2 + dir_name + '\\')
