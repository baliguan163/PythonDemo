    ...
    for pageCount in pageCounts:
        ...
        for i in range(int(pageCount)):
            if i != 0:
                ...
                getImgUrlByLink(newLink, keyName, title,pageCount,i)
            else:
                getImgUrlByLink(link, keyName, title,pageCount,i)
                pass
            pass
...
def saveImgToLocal(imgUrl, keyName, title,pageCount,pageIndex):
    path = root_dir + "/" + keyName + "/" + title + "/"
    if not os.path.isdir(path):
        os.makedirs(path)
    path += title + str(pageIndex + 1) + '.jpg'
    # print(path)
    if os.path.exists(path):
        print(keyName + '下的' + title + '中的第' + str(pageCount) + "->" + str(pageIndex + 1) + '图片已经存在不下载-->' + imgUrl)
    else:
        try:
            headers = {
            ...
            print(keyName + ' 下的 ' + title + ' 中的第' + str(pageCount) + "->" + str(pageIndex + 1) + '张图片下载完成-->' + imgUrl)
        except requests.exceptions.ConnectionError:
            print(keyName + '下的' + title + '中的第' + str(pageCount) + "->" +  str(pageIndex + 1) + '张图片下载【错误】------------')
        fp = open(path, 'wb')
